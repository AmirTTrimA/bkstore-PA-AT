...
import { pic1, pic2, pic3, ppic1 } from '../../Constants';
...
export default function Home() {
  ...
  const [sliderItems2, setSliderItems2] = useState([]);
  ...
  const slider_items = sliderItems;
  const slider_items2 = sliderItems2;
  ...
  useEffect(() => {
    const loadBooks = async () => {
      try {
        const [newBooks, allBooks] = await Promise.all([
        ...
        // --- Top Offer / New Arrivals ---
        setSliderItems(newBooks.map(mapBook));
  ...
  if (loadingBooks) {
    return <div className="p-5 text-center">Loading home page...</div>;
  }
