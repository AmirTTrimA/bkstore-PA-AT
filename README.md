# Backend Documentation — Bookstore E-Commerce Platform
### Phase 1, 2 & 3 Technical Report

**Team:** BE Dev (Backend) · FE Dev (Frontend)  
**Project:** bkstore-PA-AT  
**Repository:** Public monorepo with `backend/` and `frontend/` directories
...
| Method | Endpoint | Auth Required | Description |
|--------|----------|:---:|-------------|
| GET | `/api/v1/recommendations/<book_id>/` | No | Retrieve books similar to the specified book. |
| GET | `/api/v1/recommendations/user/` | Yes | Retrieve personalized recommendations based on user preferences and purchase history. |

---

### 4.3 Pricing Endpoints (`/api/v1/pricing/`)

| Method | Endpoint | Auth Required | Description |
|--------|----------|:---:|-------------|
| GET | `/api/v1/pricing/plans/` | No | List all active subscription plans. |
| GET | `/api/v1/pricing/subscription/` | Yes | Retrieve the authenticated user's active subscription and benefits. |
| POST | `/api/v1/pricing/validate/` | No | Validate a coupon code and return its current validity. |
| GET | `/api/v1/pricing/books/<book_id>/prices/` | No | Retrieve the historical pricing records for a book. |
| POST | `/api/v1/pricing/books/<book_id>/prices/` | Yes* | Create a new historical price entry for a book. Automatically expires the previous active price. |

> *Administrative endpoint.

---

### 4.4 Publishing Endpoints (`/api/v1/publishing/`)

The publishing API is a publisher-facing API for organization membership, proposal review, and proposal submission. All protected publishing endpoints require authentication and active membership in the relevant publisher.

#### Publisher and Proposal Access
...
| Method | Endpoint | Auth Required | Description |
|--------|----------|:---:|-------------|
| POST | `/api/v1/publishing/proposals/book-create/` | Yes | Submit a book creation proposal. |
| POST | `/api/v1/publishing/proposals/book-update/` | Yes | Submit a book update proposal. |
| POST | `/api/v1/publishing/proposals/book-delete/` | Yes | Submit a request to delete a book. |
...
The publishing API uses two complementary authorization checks:

- **Publisher-level authorization:** verifies that the authenticated user is an active member of the publisher identified by the URL or request.
- **Proposal-level authorization:** verifies that the authenticated user is an active member of the publisher that owns the requested proposal.

The publisher-facing detail serializer deliberately omits internal moderation information such as reviewer identity and unrestricted administrative review notes.

### 4.5 Cart & Checkout Endpoints (`/api/v1/cart/`)
